...
void BBR2() {if((value & 0x04) == 0) cpu.pc = addr;}
void BBR3() {if((value & 0x08) == 0) cpu.pc = addr;}
void BBR4() {if((value & 0x10) == 0) cpu.pc = addr;}
void BBR5() {if((value & 0x20) == 0) cpu.pc = addr;}
void BBR6() {if((value & 0x40) == 0) cpu.pc = addr;}
void BBR7() {if((value & 0x80) == 0) cpu.pc = addr;}

void BBS0() {if(value & 0x01) cpu.pc = addr;}
void BBS1() {if(value & 0x02) cpu.pc = addr;}
void BBS2() {if(value & 0x04) cpu.pc = addr;}
void BBS3() {if(value & 0x08) cpu.pc = addr;}
void BBS4() {if(value & 0x10) cpu.pc = addr;}
void BBS5() {if(value & 0x20) cpu.pc = addr;}
void BBS6() {if(value & 0x40) cpu.pc = addr;}
void BBS7() {if(value & 0x80) cpu.pc = addr;}
...
void TSX() {cpu.x = cpu.sp;SET_N(cpu.x); SET_Z(cpu.x);}
...
struct instruction instructions[256] = {
    ...
    {"STZ",&STZ,&ABS,4},{"STA",&STA,&ABX,4},{"STZ",&STZ,&ABX,4},{"BBS1",&BBS1,&ZPR,2},
    ...
    {"LDY",&LDY,&ABS,2},{"LDA",&LDA,&ABS,4},{"LDX",&LDX,&ABS,4},{"BBS2",&BBS2,&ZPR,2},
    ...
    {"PHP",&PHP,&STK,3},{"ORA",&ORA,&IMM,2},{"ASL",&ASL,&ACC,2},{"XXX",&XXX,&XXX,0},
    {"XXX",&XXX,&XXX,0},{"ORA",&ORA,&ABS,4},{"ASL",&ASL,&ABS,4},{"XXX",&XXX,&XXX,0},
    {"BPL",&BPL,&PCR,2},{"ORA",&ORA,&ZIY,5},{"ORA",&ORA,&ZPI,5},{"XXX",&XXX,&XXX,0},
    {"XXX",&XXX,&XXX,0},{"ORA",&ORA,&ZPX,4},{"ASL",&ASL,&ZPX,4},{"XXX",&XXX,&XXX,0},
    {"CLC",&CLC,&IMP,2},{"ORA",&ORA,&ABY,4},{"INC",&INC,&ACC,2},{"XXX",&XXX,&XXX,0},
    {"XXX",&XXX,&XXX,0},{"ORA",&ORA,&ABX,4},{"ASL",&ASL,&ABX,4},{"XXX",&XXX,&XXX,0},
    {"JSR",&JSR,&ABS,4},{"AND",&AND,&ZII,6},{"XXX",&XXX,&XXX,0},{"XXX",&XXX,&XXX,0},
    {"BIT",&BIT,&ZP0,3},{"AND",&AND,&ZP0,3},{"ROL",&ROL,&ZP0,3},{"XXX",&XXX,&XXX,0},
    {"PLP",&PLP,&STK,3},{"AND",&AND,&IMM,2},{"ROL",&ROL,&ACC,2},{"XXX",&XXX,&XXX,0},
    {"BIT",&BIT,&ABS,4},{"AND",&AND,&ABS,4},{"ROL",&ROL,&ABS,4},{"XXX",&XXX,&XXX,0},
    {"BMI",&BMI,&PCR,2},{"AND",&AND,&ZIY,5},{"AND",&AND,&ZPI,5},{"XXX",&XXX,&XXX,0},
    {"XXX",&XXX,&XXX,0},{"AND",&AND,&ZPX,4},{"ROL",&ROL,&ZPX,4},{"XXX",&XXX,&XXX,0},
    {"SEC",&SEC,&IMP,2},{"AND",&AND,&ABY,4},{"DEC",&DEC,&ACC,2},{"XXX",&XXX,&XXX,0},
    {"BIT",&BIT,&ABX,4},{"AND",&AND,&ABX,4},{"ROL",&ROL,&ABX,4},{"XXX",&XXX,&XXX,0},
    {"RTI",&RTI,&STK,3},{"EOR",&EOR,&ZII,6},{"XXX",&XXX,&XXX,0},{"XXX",&XXX,&XXX,0},
    {"XXX",&XXX,&XXX,0},{"EOR",&EOR,&ZP0,3},{"LSR",&LSR,&ZP0,3},{"XXX",&XXX,&XXX,0},
    {"PHA",&PHA,&STK,3},{"EOR",&EOR,&IMM,2},{"LSR",&LSR,&ACC,2},{"XXX",&XXX,&XXX,0},
    {"JMP",&JMP,&ABS,4},{"EOR",&EOR,&ABS,4},{"LSR",&LSR,&ABS,4},{"XXX",&XXX,&XXX,0},
    {"BVC",&BVC,&PCR,2},{"EOR",&EOR,&ZIY,5},{"EOR",&EOR,&ZPI,5},{"XXX",&XXX,&XXX,0},
    {"XXX",&XXX,&XXX,0},{"EOR",&EOR,&ZPX,4},{"LSR",&LSR,&ZPX,4},{"XXX",&XXX,&XXX,0},
    {"CLI",&CLI,&IMP,2},{"EOR",&EOR,&ABY,4},{"XXX",&XXX,&XXX,0},{"XXX",&XXX,&XXX,0},
    {"XXX",&XXX,&XXX,0},{"EOR",&EOR,&ABX,4},{"LSR",&LSR,&ABX,4},{"XXX",&XXX,&XXX,0},
    {"RTS",&RTS,&STK,3},{"ADC",&ADC,&ZII,6},{"XXX",&XXX,&XXX,0},{"XXX",&XXX,&XXX,0},
    {"XXX",&XXX,&XXX,0},{"ADC",&ADC,&ZP0,3},{"ROR",&ROR,&ZP0,3},{"XXX",&XXX,&XXX,0},
    {"PLA",&PLA,&STK,3},{"ADC",&ADC,&IMM,2},{"ROR",&ROR,&ACC,2},{"XXX",&XXX,&XXX,0},
    {"JMP",&JMP,&IND,6},{"ADC",&ADC,&ABS,4},{"ROR",&ROR,&ABS,4},{"XXX",&XXX,&XXX,0},
    {"BVS",&BVS,&PCR,2},{"ADC",&ADC,&ZIY,5},{"ADC",&ADC,&ZPI,5},{"XXX",&XXX,&XXX,0},
    {"XXX",&XXX,&XXX,0},{"ADC",&ADC,&ZPX,4},{"ROR",&ROR,&ZPX,4},{"XXX",&XXX,&XXX,0},
    {"SEI",&SEI,&IMP,2},{"ADC",&ADC,&ABY,4},{"XXX",&XXX,&XXX,0},{"XXX",&XXX,&XXX,0},
    {"JMP",&JMP,&ABI,6},{"ADC",&ADC,&ABX,4},{"ROR",&ROR,&ABX,4},{"XXX",&XXX,&XXX,0},
    {"XXX",&XXX,&XXX,0},{"STA",&STA,&ZII,6},{"XXX",&XXX,&XXX,0},{"XXX",&XXX,&XXX,0},
    {"STY",&STY,&ZP0,3},{"STA",&STA,&ZP0,3},{"STX",&STX,&ZP0,3},{"XXX",&XXX,&XXX,0},
    {"DEY",&DEY,&IMP,2},{"XXX",&XXX,&XXX,0},{"TXA",&TXA,&IMP,2},{"XXX",&XXX,&XXX,0},
    {"STY",&STY,&ABS,4},{"STA",&STA,&ABS,4},{"STX",&STX,&ABS,4},{"XXX",&XXX,&XXX,0},
    {"BCC",&BCC,&PCR,2},{"STA",&STA,&ZIY,5},{"STA",&STA,&ZPI,5},{"XXX",&XXX,&XXX,0},
    {"STY",&STY,&ZPX,4},{"STA",&STA,&ZPX,4},{"STX",&STX,&ZPY,4},{"XXX",&XXX,&XXX,0},
    {"TYA",&TYA,&IMP,2},{"STA",&STA,&ABY,4},{"TXS",&TXS,&IMP,2},{"XXX",&XXX,&XXX,0},
    {"XXX",&XXX,&XXX,0},{"STA",&STA,&ABX,4},{"XXX",&XXX,&XXX,0},{"XXX",&XXX,&XXX,0},
    {"LDY",&LDY,&IMM,2},{"LDA",&LDA,&ZII,6},{"LDX",&LDX,&IMM,2},{"XXX",&XXX,&XXX,0},
    {"LDY",&LDY,&ZP0,3},{"LDA",&LDA,&ZP0,3},{"LDX",&LDX,&ZP0,3},{"XXX",&XXX,&XXX,0},
    {"TAY",&TAY,&IMP,2},{"LDA",&LDA,&IMM,2},{"TAX",&TAX,&IMP,2},{"XXX",&XXX,&XXX,0},
    {"LDY",&LDY,&ABS,2},{"LDA",&LDA,&ABS,4},{"LDX",&LDX,&ABS,4},{"XXX",&XXX,&XXX,0},
    {"BCS",&BCS,&PCR,2},{"LDA",&LDA,&ZIY,5},{"LDA",&LDA,&ZPI,5},{"XXX",&XXX,&XXX,0},
    {"LDY",&LDY,&ZPX,4},{"LDA",&LDA,&ZPX,4},{"LDX",&LDX,&ZPY,4},{"XXX",&XXX,&XXX,0},
    {"CLV",&CLV,&IMP,2},{"LDA",&LDA,&ABY,4},{"TSX",&TSX,&IMP,2},{"XXX",&XXX,&XXX,0},
    {"LDY",&LDY,&ABX,4},{"LDA",&LDA,&ABX,4},{"LDX",&LDX,&ABY,4},{"XXX",&XXX,&XXX,0},
    {"CPY",&CPY,&IMM,2},{"CMP",&CMP,&ZII,6},{"XXX",&XXX,&XXX,0},{"XXX",&XXX,&XXX,0},
    {"CPY",&CPY,&ZP0,3},{"CMP",&CMP,&ZP0,3},{"DEC",&DEC,&ZP0,3},{"XXX",&XXX,&XXX,0},
    {"INY",&INY,&IMP,2},{"CMP",&CMP,&IMM,2},{"DEX",&DEX,&IMP,2},{"XXX",&XXX,&XXX,0},
    {"CPY",&CPY,&ABS,4},{"CMP",&CMP,&ABS,4},{"DEC",&DEC,&ABS,4},{"XXX",&XXX,&XXX,0},
    {"BNE",&BNE,&PCR,2},{"CMP",&CMP,&ZIY,5},{"CMP",&CMP,&ZPI,5},{"XXX",&XXX,&XXX,0},
    {"XXX",&XXX,&XXX,0},{"CMP",&CMP,&ZPX,4},{"DEC",&DEC,&ZPX,4},{"XXX",&XXX,&XXX,0},
    {"CLD",&CLD,&IMP,2},{"CMP",&CMP,&ABY,4},{"XXX",&XXX,&XXX,0},{"XXX",&XXX,&XXX,0},
    {"XXX",&XXX,&XXX,0},{"CMP",&CMP,&ABX,4},{"DEC",&DEC,&ABX,4},{"XXX",&XXX,&XXX,0},
    {"CPX",&CPX,&IMM,2},{"SBC",&SBC,&ZII,6},{"XXX",&XXX,&XXX,0},{"XXX",&XXX,&XXX,0},
    {"CPX",&CPX,&ZP0,3},{"SBC",&SBC,&ZP0,3},{"INC",&INC,&ZP0,3},{"XXX",&XXX,&XXX,0},
    {"INX",&INX,&IMP,2},{"SBC",&SBC,&IMM,2},{"NOP",&NOP,&IMP,2},{"XXX",&XXX,&XXX,0},
    {"CPX",&CPX,&ABS,4},{"SBC",&SBC,&ABS,4},{"INC",&INC,&ABS,4},{"XXX",&XXX,&XXX,0},
    {"BEQ",&BEQ,&PCR,2},{"SBC",&SBC,&ZIY,5},{"SBC",&SBC,&ZPI,5},{"XXX",&XXX,&XXX,0},
    {"XXX",&XXX,&XXX,0},{"SBC",&SBC,&ZPX,4},{"INC",&INC,&ZPX,4},{"XXX",&XXX,&XXX,0},
    {"SED",&SED,&IMP,2},{"SBC",&SBC,&ABY,4},{"XXX",&XXX,&XXX,0},{"XXX",&XXX,&XXX,0},
    {"XXX",&XXX,&XXX,0},{"SBC",&SBC,&ABX,4},{"INC",&INC,&ABX,4},{"XXX",&XXX,&XXX,0}
#endif
};
